...
void champsim::plain_printer::print(O3_CPU::stats_type stats)
{
  ...
  fmt::print(stream, "\n{} cumulative IPC: {:.4g} instructions: {} cycles: {} instr_foot_print: {} data_foot_print: {}\n", stats.name, std::ceil(stats.instrs()) / std::ceil(stats.cycles()),
             stats.instrs(), stats.cycles(), stats.instr_foot_print.size(), stats.data_foot_print.size());
